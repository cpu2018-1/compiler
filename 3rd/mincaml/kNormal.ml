...
let insert_let (e, t) k = (* letを挿入する補助関数 (caml2html: knormal_insert) *)
 ...
 let rec g env = function (* K正規化ルーチン本体 (caml2html: knormal_g) *) 
  ...
  | Syntax.Let((x, t), e1, e2, d) ->
      ...
      Let((x, t), e1', e2'), t2
  | Syntax.Var(x, d) when M.mem x env -> 
      Var(x), M.find x env
  ...
  | Syntax.Store_hp(e, _) ->
      insert_let (g env e)
        (fun x -> Store_hp(x), Type.Unit)
  | Syntax.FStore_hp(e, _) ->
      insert_let (g env e)
        (fun x -> FStore_hp(x), Type.Unit)
let f e = 
  fst (g M.empty e)
