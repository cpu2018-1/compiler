...
and g' env fenv aenv = function (* 各命令の16bit即値最適化 (caml2html: simm13_gprime) *)
  ...
  | Lwz(x, V(y)) when M.mem x aenv && M.mem y env -> Lwz(Asm.reg_zero, C(M.find (M.find x aenv) Glbarray.table + M.find y env))
  | Lwz(x, V(y)) when M.mem y env -> Lwz(x, C(M.find y env))
  | Stw(x, y, V(z)) when M.mem x env && M.find x env = 0 && M.mem y aenv && M.mem z env -> Stw(Asm.reg_zero, Asm.reg_zero, C(M.find (M.find y aenv) Glbarray.table + M.find z env))
  | Stw(x, y, V(z)) when M.mem y aenv && M.mem z env -> Stw(x, Asm.reg_zero, C(M.find (M.find y aenv) Glbarray.table + M.find z env))
  | Stw(x, y, V(z)) when M.mem x fenv && M.find x fenv = 0.0 && M.mem z env -> Stw(Asm.reg_zero, y, C(M.find z env))
  | Stw(x, y, V(z)) when M.mem z env -> Stw(x, y, C(M.find z env))
  | Stw(x, y, V(z)) when M.mem x env && M.find x env = 0 -> Stw(Asm.reg_zero, y, V(z))
