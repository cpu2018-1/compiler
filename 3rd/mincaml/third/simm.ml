...
and g' env fenv aenv = function (* 各命令の16bit即値最適化 (caml2html: simm13_gprime) *)
  ...
  | Sw(x, y, V(z)) when M.mem x env && M.find x env = 0 && M.mem y aenv && M.mem z env -> Sw(Asm.reg_zero, Asm.reg_zero, C(M.find (M.find y aenv) Glbarray.table + M.find z env))
  | Sw(x, y, V(z)) when M.mem y aenv && M.mem z env -> Sw(x, Asm.reg_zero, C(M.find (M.find y aenv) Glbarray.table + M.find z env))
  | Sw(x, y, V(z)) when M.mem x fenv && M.find x fenv = 0.0 && M.mem z env -> Sw(Asm.reg_zero, y, C(M.find z env))
  | Sw(x, y, V(z)) when M.mem z env -> Sw(x, y, C(M.find z env))
  | Sw(x, y, V(z)) when M.mem x env && M.find x env = 0 -> Sw(Asm.reg_zero, y, V(z))
